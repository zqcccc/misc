...
def main():
    ...
    # ---- 代理 vs 真数据：唯一变量是数据 ----
    pp = os.path.join(V, "ext_summary_proxy.json")
    if os.path.exists(pp):
        pr = {x["key"].replace("_proxy", ""): x for x in json.load(open(pp))["策略"]}
        print("\n### 代理数据 vs 真实数据（同一份代码，唯一变量是数据）\n")
        print("| 策略 | 代理年化 | 真数据年化 | 代理对等权超额 | 真数据超额 | 代理换手 | 真换手 | 代理成本%/年 | 真成本%/年 |")
        print("|---|---|---|---|---|---|---|---|---|")
        for r in rows:
            q = pr.get(r["key"])
            if not q:
                continue
            print(f"| {r['title']} | {q['年化%']}% | **{r['年化%']}%** | {q['对全池等权超额%']}% | "
                  f"**{r['对全池等权超额%']}%** | {q['年化换手(倍)']}× | {r['年化换手(倍)']}× | "
                  f"{q['成本拖累%/年']} | {r['成本拖累%/年']} |")
