...
> ## ⛔ 已归档废弃（2026-09-11）
>
> **不要再启动 daemon、不要加品种、不要改参数、不要改代码。**
> 本目录及 `btc_ema_15m/`、`eth_sol_bnb_15m/` 下的 EMA 趋势反手策略已停止维护，仅作研究档案保留。
> `strategies.json` 顶部 `_archived` 字段是权威标记。
> 最后运行时间 **2026-09-03 14:32**，此后 15 个品种的持仓、价格、信号全部冻结，
> 快照数据不具备任何交易参考价值。

---

一个入口、一份配置、一套 daemon 实现，统一管理所有品种的 EMA 趋势反手策略实时监控。（历史说明，归档前撰写）
