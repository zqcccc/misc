...
研究方法采用三层漏斗：**快速证伪 → 可交易验证 → 候选晋级**。探索允许反复迭代，
但信息/决策/成交/PnL 时序、全部搜索记账、滚动样本外和组合增量是晋级硬要求。
物理隔离 TEST 是可选增强，不是研究工作的中心。
...
├── GLOSSARY.md      ★ 术语表：alpha / NW t / DSR / 置换分位 各自在问什么、多少算过线
├── rounds/          本轮台账：候选池全表 + 打分卡（不进对话汇报，只留指针）
...
按机理、成交时点、成本余量和组合增量撒出 8~12 个候选，快速证伪后才把预算投向幸存者。
...
./qr audit                                  # 检查卡片；PASS 必须有严格 v2 机器报告
