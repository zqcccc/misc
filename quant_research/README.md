...
```
读 quant_research/PROMPT.md 并严格照它执行。本轮探索方向：<方向>；市场：<market>；预算：<N 个假设>；我的 agent 名：<name>
```

方向留空即开放式探索。多个 agent 可同时跑。
...
./qr claim "一句话假设" --family reversal --market cn_stock --tags a,b --agent alice
...
1. **先查重后占坑**：`qr search` 无命中，或读完命中卡确认机理/口径不同，才可 `qr claim`。
